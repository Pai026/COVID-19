...
df1.to_excel("../Data/output1.xlsx")
#print(df1)
...
for idx, row in merged.iterrows():
    if not math.isnan(row['cases']) and not math.isnan(row['cases']):
        ...
        popup='<STRONG>'+'State '+row['province']+'<br>'+'Cases-'+str(int((row['cases'])))+'<br>'+'Deaths-'+str(int((row['deaths'])))+'<br>'+'Discharged-'+str(int(row['discharged']))+'<br>'+'Helpline-'+str(row['helpline'])+'</STRONG>'
