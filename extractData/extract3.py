...
m_3.add_child(fg)
fg1=folium.FeatureGroup(name='Circle', show=False)
m_3.add_child(fg1)
...
for idx, row in data.iterrows():
    if (str(row['Province/State'])!="nan"):
            ...
            radius=15000 + int(row['Confirmed']) * 2500
            mc1.add_child(Circle(location=location,popup=popup,radius=radius,fill_colour='#FF0000',fill=True))
    else:
        location=[row['Latitude'],row['Longitude']]
